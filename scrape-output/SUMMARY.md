...
- **Philip Short — name spelling.** Final decision per Stone (reversing earlier guidance): the correct spelling is **"Philip" with one L**, matching the live site. An earlier note here claimed two Ls was correct; that was reversed. `copy.md`, `camps.json`, the derived `images.json`/`videos.json` data keys, the scrape scripts, and the shipped frontend now all use the one-L spelling. `MSI-AMENDMENT.md` is the authoritative tiebreaker. Note: the participant CSVs in `data/mailchimp-import/` contain unrelated real people named "Phillip"/"Phillips" — those are left as-is.
